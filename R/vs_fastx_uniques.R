...
#' @param sizein Decides if abundance annotations present in sequence headers in \code{fastx_input} should be taken into account.
#' @param sizeout Decides if abundance annotations should be added to headers in output table or output file (\code{fastx_output}).
...
#' @param fastq_qout_max If \code{TRUE}, the quality score will be the highest (best) quality score observed in each position.
#'
