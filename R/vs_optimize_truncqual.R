...
#' the optimization process. Set to \code{NULL} to use all reads.
...
#' @param truncqual_range (Optional). A numeric vector of \code{truncqual}
...
vs_optimize_truncqual <- function(fastq_input,
  ...
  if (plot_title) {
    ...
    title <- paste0(max(res.df$merged_read_pairs), " merged pairs at truncqual ",
                    optimal_truncqual, " (", sample_info, ", size > ", min_size, ")")
  } else {
    title <- ""
  }
