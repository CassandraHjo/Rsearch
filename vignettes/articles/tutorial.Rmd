...
Before applying trimming and filtering, we can merge the raw reads to establish a baseline understanding of read overlap and the number of successfully merged reads. Here we try to merge all the reads we have got for the first sample, without any trimming and filtering. The `vs_merging_lengths` function can be used to plot some merging statistics:
...
We now want to optimize the trimming of the reads in order to get the optimal merging results. We do not necessarily just want to merge as many as possible, but rather merge high-quality reads. For this purpose, we have created two functions that iterate over a range of `truncqual` and `truncee_rate` values used during trimming, to identify the settings that yield the best outcome. Remember that the optimization is based on reads from one sample, but our experience is that reads from the same sequencing run tend to get the same results. We still assume that sequencing errors are more likely to occur in singletons, and we therefore choose to look at merged reads with a copy number of 2 or more. We assume that the more of these merged sequences, the better the merging is. During this optimization of truncation, the reads are also filtered to remove the reads with average expected error greater than 0.01 (default). 
