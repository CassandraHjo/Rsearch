...
Before trimming and filtering, we can try merging the raw reads to get a baseline understanding of read overlap and quality. Here we try to merge all the reads we have got for the first sample, without any trimming and filtering. The `vs_merging_lengths` function can be used to plot some merging statistics:
...
The results from merging the raw reads above includes singletons, merged read pairs with a copy number of 1. If we assume that sequencing errors are more likely to occur among singletons and remove them, we are left with approximately 49% (33 259 out of 67 857) successfully merged read pairs when merging the raw read pairs.  
...
We now want to optimize the trimming of the reads in order to get the optimal merging results. We do not neccecarily just want to merge as many as possible, but rather merge high-quality reads. For this purpose, we have created two functions that iterate over a range of `truncqual` and `truncee_rate` values used during trimming, to identify the settings that yield the best outcome. We still assume that sequencing errors are more likely to occur in singletons, and we therefore choose to look at merged reads with a copy number of 2 or more. We assume that the more of these merged sequences, the better the merging is. During this optimization of truncation, the reads are also filtered to remove the reads with average expected error greater than 0.01 (default). 
...
```{r optimize_truncqual, echo=FALSE, fig.keep='all', message=FALSE, warning=FALSE, results='hide', eval=FALSE}
...
Although this number is just slightly higher compared to the results from the raw merging, the important difference is that only high-quality merged reads with copy number above 2 and an average expected error bellow 0.01 are considered here. Thus, trimming with `truncqual = 7` effectively retains higher-confidence reads at the cost of discarding lower-quality ones, which is exactly what we want.
...
```{r optimize_truncee_rate, echo=FALSE, fig.keep='all', message=FALSE, warning=FALSE, results='hide', eval=FALSE}
...
all.tbl <- read_tsv("path/to/all_tbl.txt.zip") # Change me
```
